...
class HBRequestWrapperTests: XCTestCase {
    ...
    func testBaseProperties() {
        let wrapper: HTTPRequest = MockRequest.create(url: "https://127.0.0.1:8080/xyz")
    ...
    func testQueryParameters() {
        let wrapper: HTTPRequest = MockRequest.create(url: "https://127.0.0.1/xyz?xxx=5&yyy=6")
    ...
    func testQueryParametersPercentEncoding() {
        let wrapper: HTTPRequest = MockRequest.create(url: "https://127.0.0.1/xyz?xxx=Hello%20world")
        expect(wrapper.query) == "xxx=Hello%20world"
        expect(wrapper.queryParameters["xxx"]) == "Hello world"
    }

    func testQueryParametersPlusEncoding() {
        let wrapper: HTTPRequest = MockRequest.create(url: "https://127.0.0.1/xyz?xxx=Hello+world")
        expect(wrapper.query) == "xxx=Hello+world"
        expect(wrapper.queryParameters["xxx"]) == "Hello+world"
    }

    func testQueryParametersMultiples() {
        let wrapper: HTTPRequest = MockRequest.create(url: "https://127.0.0.1/xyz?xxx=5&xxx=6&xxx=7")
        expect(wrapper.queryParameters["xxx"]) == ["5", "6", "7"]
    }

    func testJSONContent() {
        let wrapper: HTTPRequest = MockRequest.create(url: "https://127.0.0.1",
